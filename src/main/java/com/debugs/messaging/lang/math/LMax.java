 ...
 */
public class LMax extends LOpNum<TNumber> {
    
    public LMax(LObject value1, LObject value2) {
        super(value1, value2);
    }

    @Override
    protected TNumber execute(int value1, int value2) throws Exception {
        return TInteger.newInstance(Math.max(value1, value2));
    }

    @Override
    protected TNumber execute(double value1, double value2) throws Exception {
        return TDecimal.newInstance(Math.max(value1, value2));
    }
}
